...
```{r}
df_combined %>% ggplot(., aes(x=Frame, y=Mean, group = ROI)) + geom_line() +facet_grid(Ch~source)
```
...
```{r}
ggplot(df_lifetime, aes(x=Time, y=Lifetime, group = ROI)) + geom_line() +facet_wrap(~source, ncol=4)
```
